...
describe('translations-controller', function() {
    ...
    it('GET /rest/v1/translations', function() {
        const params = {
    ...
    it('POST /rest/v1/translations/clear-cache', function() {
        const params = {
        };
        ...
        return Promise.resolve().then(() => {
            return client.restRequest({
                method: 'POST',
                path: `/rest/v1/translations/clear-cache`,
            });
        }).then(response => {
            // OK
            assert.strictEqual(response.status, 200);
    ...
    it('GET /rest/v1/translations/public/{namespaces}', function() {
        const params = {
            browser: false, // in = query, description = browser, required = false, type = boolean, default = false, enum = 
        ...
        return Promise.resolve().then(() => {
            ...
            return publicClient.restRequest({
                method: 'GET',
                path: `/rest/v1/translations/public/${params.namespaces}`,
                params: {
                    browser: params.browser,
                    language: params.language,
                    server: params.server
    ...
    it('GET /rest/v1/translations/{namespaces}', function() {
        const params = {
            browser: false, // in = query, description = browser, required = false, type = boolean, default = false, enum = 
            language: 'en-US', // in = query, description = Language for translation (must have language pack installed), required = false, type = string, default = , enum = 
        ...
        return Promise.resolve().then(() => {
            return client.restRequest({
                method: 'GET',
                path: `/rest/v1/translations/${params.namespaces}`,
                params: {
                    browser: params.browser,
                    language: params.language,
                    server: params.server
