 ...
 *
 */
public class IdPointValueStatisticsQuantizerJsonCallback extends PointValueTimeJsonWriter
        implements ParentStatisticsQuantizerCallback {

    private static final Log LOG =
            LogFactory.getLog(IdPointValueStatisticsQuantizerJsonCallback.class);
     ...
     */
    @Override
    public void closePeriod(Map<Integer, StatisticsGenerator> periodStatsMap,
            ...
            while (it.hasNext()) {
                ...
                if (stats instanceof ValueChangeCounter) {
                    ...
                    switch (rollup) {
                        case ALL:
                            ...
                            if (vo.getPointLocator().getDataTypeId() == DataTypes.IMAGE)
                                this.writeImageValue(statisticsGenerator.getFirstValue(),
                                        statisticsGenerator.getFirstTime(), periodStartTime, vo,
                                        RollupEnum.FIRST.name());
                            else
                                this.writeDataValue(periodStartTime,
                                        statisticsGenerator.getFirstValue(), vo,
                                        RollupEnum.FIRST.name());

                            if (vo.getPointLocator().getDataTypeId() == DataTypes.IMAGE)
                                this.writeImageValue(statisticsGenerator.getLastValue(),
                                        statisticsGenerator.getLastTime(), periodStartTime, vo,
                                        RollupEnum.LAST.name());
                            else
                                this.writeDataValue(periodStartTime,
                                        statisticsGenerator.getLastValue(), vo,
                                        RollupEnum.LAST.name());
                            this.jgen.writeNumberField(RollupEnum.COUNT.name(),
                                    statisticsGenerator.getCount());
                            this.jgen.writeEndObject();
                            break;
                        case START:
                            if (vo.getPointLocator().getDataTypeId() == DataTypes.IMAGE)
                                this.writeImageValue(statisticsGenerator.getStartValue(),
                                        periodStartTime, periodStartTime, vo, vo.getXid());
                            else
                                this.writeDataValue(periodStartTime,
                                        statisticsGenerator.getStartValue(), vo, vo.getXid());
                            break;
                        case FIRST:
                            if (vo.getPointLocator().getDataTypeId() == DataTypes.IMAGE)
                                this.writeImageValue(statisticsGenerator.getFirstValue(),
                                ...
                                this.writeDataValue(periodStartTime,
                                        statisticsGenerator.getFirstValue(), vo, vo.getXid());
                            break;
                        case LAST:
                            if (vo.getPointLocator().getDataTypeId() == DataTypes.IMAGE)
                                this.writeImageValue(statisticsGenerator.getLastValue(),
                                ...
                                this.writeDataValue(periodStartTime,
                                        statisticsGenerator.getLastValue(), vo, vo.getXid());
                            break;
                        case COUNT:
                    ...
                    throw new ShouldNeverHappenException(
                            "Unsuported statistics type: " + stats.getClass().getName());
                }
            }
            this.jgen.writeEndObject();
        } catch (IOException e) {
