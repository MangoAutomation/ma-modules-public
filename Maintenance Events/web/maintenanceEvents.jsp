...
<%@page import="com.serotonin.m2m2.maintenanceEvents.MaintenanceEventVO"%>
...
<tag:page dwr="MaintenanceEventsDwr" onload="init">
  <script type="text/javascript">
    ...
    function showMaintenanceEvent(meId) {
        // Check if the maintenance event exists. It may have been deleted, but still referenced by an event.
        if (!$("me"+ meId +"Img"))
            return;
        ...
        MaintenanceEventsDwr.saveMaintenanceEvent(editingMaintenanceEvent.id, $get("xid"), $get("dataSourceId"), 
        		...
        		function(response) {
                    if (response.hasMessages)
                        ...
                        if (editingMaintenanceEvent.id == ${newId}) {
                            stopImageFader($("me"+ editingMaintenanceEvent.id +"Img"));
    ...
    function updateToggle(active) {
        var display = editingMaintenanceEvent.id != ${newId};
    	
    	if (active)
            updateImg("toggleImg", "${modulePath}/web/hammer_activated.png", "<sst:i18n key="maintenanceEvents.activated" escapeDQuotes="true"/>", display);
    	else
            updateImg("toggleImg", "${modulePath}/web/hammer_deactivated.png", "<sst:i18n key="maintenanceEvents.deactivated" escapeDQuotes="true"/>", display);
    }
        ...
        <div class="borderDiv">
          <table width="100%">
            <tr>
              ...
              <td align="right"><tag:img png="add" title="maintenanceEvents.addMe"
                      onclick="showMaintenanceEvent(${newId})" id="me${newId}Img"/></td>
            </tr>
          </table>
          <table id="maintenanceEventsTable">
            <tbody id="me_TEMPLATE_" onclick="showMaintenanceEvent(getMangoId(this))" class="ptr" style="display:none">
              <tr>
                <td><tag:img id="me_TEMPLATE_Img" src="${modulePath}/web/hammer.png" title="maintenanceEvents.me"/></td>
                    ...
                    <td id="activeHourData">
                      <select id="activeHour">
                        <c:forEach begin="0" end="23" var="i">
                          <option>${m2m2:padZeros(i, 2)}</option>
                        </c:forEach>
                      </select>
                    </td>
